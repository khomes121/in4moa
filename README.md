...
슬러그(URL)는 고정, 라벨·정체성은 2026-06-10 개편:

| 슬러그 | 라벨 | 다루는 영역 |
|---|---|---|
| `realestate` | 🏠 부동산 | 청약·분양·매매·전월세 |
| `tax` | 🧾 세금 | 취득세·양도세·종부세·증여세 |
| `loan` | 🏦 대출·금융 | 디딤돌·버팀목·DSR |
| `subsidy` | 💰 지원금·복지 | 근로장려금·주거급여·청년 지원 |
| `calculator` | 🧮 계산기·가이드 | 계산기 활용 가이드 글 (도구 본체는 /calculator/ 페이지) |
| `business` | 🏪 창업·자영업 | 사업자등록·부가세·정책자금 |
| `news` | 📌 정책 뉴스 | 생활에 영향 있는 정책 동향 |

카테고리 메타데이터: `src/consts.ts` (`CATEGORIES`)

## 브랜드 (2026-06-10 리디자인)

- **팔레트**: 딥 그린(`--color-primary-700: #0f5c40`, 돈·신뢰) + 골드 앰버 액센트 + 웜 페이퍼 그레이
- **마크**: "흩어진 정보를 모아 쌓는" 3단 스택 (favicon.svg · 헤더·푸터 인라인 SVG · OG 이미지 공통)
- OG 이미지 수정 시: `public/og-default.svg` 편집 → sharp 로 PNG 재생성
